...
export type CampaignType = "brand" | "generic" | "high-intent" | "competitor" | "custom";
...
export function createCampaign(name: string, campaignType?: CampaignType): Campaign {
  const c: Campaign = { id: uid(), name: name.trim(), campaignType, budgetMode: "auto", excludeFromForecast: false, createdAt: new Date().toISOString() };
...
export function updateCampaign(id: string, patch: Partial<Pick<Campaign, "name" | "campaignType" | "budgetMode" | "budgetAmount" | "defaultMatchType" | "excludeFromForecast">>): void {
